...
private:
    ...
    void cubeFaces();
    void triangulate(std::unique_ptr<std::vector<float>> points, int d0, int d1, int d2, int rows, int cols);
    void test();

    //std::unique_ptr<OpenGLShape> m_cube;
//    std::unique_ptr<Shape> m_shape;
//    std::unique_ptr<Cube> myCube;
    //does this work? with inheritance? to have it be type Shape
